...
static void timeout_cb(TimerHandle_t timer)
{
    ...
    LOGI(__func__, "%d high", (int)_dev->power_pin);
}
#endif
...
int lm3644_init2(led_dev_t *dev)
{
    ...
    else
    {
        ...
        LOGI(__func__, "set %d GPIO_DM_OUTPUT", (int)pin);
#if USE_RTOS
        int pulse_in_ms = 2;
        ...
        LOGI(TAG, "timeout_cb pulse_in_ms %d ms\n", pulse_in_ms);
